defmodule WCore.TelemetryTest do
  ...
  describe "node_metrics" do
    ...
    test "create_node_metric/2 with valid data creates a node_metric" do
      valid_attrs = %{status: "some status", total_events_processed: 42, last_payload: "some last_payload", last_seen_at: ~U[2026-03-30 18:35:00Z]}
      scope = user_scope_fixture()
      ...
      assert node_metric.last_payload == "some last_payload"
      assert node_metric.last_seen_at == ~U[2026-03-30 18:35:00Z]
    ...
    test "update_node_metric/3 with valid data updates the node_metric" do
      scope = user_scope_fixture()
      node_metric = node_metric_fixture(scope)
      update_attrs = %{status: "some updated status", total_events_processed: 43, last_payload: "some updated last_payload", last_seen_at: ~U[2026-03-31 18:35:00Z]}
      ...
      assert node_metric.last_payload == "some updated last_payload"
      assert node_metric.last_seen_at == ~U[2026-03-31 18:35:00Z]
    end
